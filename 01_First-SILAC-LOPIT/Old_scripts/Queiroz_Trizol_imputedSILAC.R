...
PSM=uniquePSM=PSMsM=tDat=PSM_D=Pep_D=Prot_D=impPep_D=impProt_D=NULL
...
for ( i in 1:NumReps ) {
  ...
  PSMsM[[i]]$imputed_Light = with(PSMsM[[i]],impute(PSMsM[[i]][,3],min))
  PSMsM[[i]]$imputed_Heavy = with(PSMsM[[i]],impute(PSMsM[[i]][,4],min))
 
  #log2/median normalization 
  tDat[[i]]=log (PSMsM[[i]][,3:6],2)
...
par(mfrow=c(3,2))
hist(PSMsM[[1]][,5],100,main="imputed Rep1 Light",xlab="Intensity",cex.main=0.8)
hist(PSMsM[[1]][,6],100,main="imputed Rep1 Heavy",xlab="Intensity",cex.main=0.8)

hist(PSMsM[[2]][,5],100,main="imputed Rep2 Light",xlab="Intensity",cex.main=0.8)
hist(PSMsM[[2]][,6],100,main="imputed Rep2 Heavy",xlab="Intensity",cex.main=0.8)

hist(PSMsM[[3]][,5],100,main="imputed Rep3 Light",xlab="Intensity",cex.main=0.8)
hist(PSMsM[[3]][,6],100,main="imputed Rep3 Heavy",xlab="Intensity",cex.main=0.8)
par(mfrow=c(1,1))

#Rrollup (without imputation)
for  (i in 1:NumReps){
  cat( "Processing replicate ", i, "\n")
...
#Rrollup (with imputation)
for  (i in 1:NumReps){
  cat( "Processing replicate ", i, "\n")
  #average PSMs from same sequences
  impPep_D[[i]] = RRollup(PSMsM[[i]][,5:6],PSMsM[[i]][,1],Mode="mean",oneHitWonders=T,center=F,minPep=1)$RolledUp
  impPep_D[[i]] = merge(impPep_D[[i]],PSMsM[[i]][,1:2],by.x=0,by.y=1)
  impPep_D[[i]] = unique(impPep_D[[i]])
  impPep_D[[i]] = impPep_D[[i]][!duplicated(impPep_D[[i]][,1]),]
  
  #proper rollup to proteins
  impProt_D[[i]] = RRollup(impPep_D[[i]][,2:3],impPep_D[[i]][,4],Mode="mean",oneHitWonders=T,center=F,minPep=1)$RolledUp
  #impProt_D[[i]] = impProt_D[[i]]-rowMeans(impProt_D[[i]],na.rm=T) # another normalization?!
}

# merge replicates
impProt_D[[1]] = cbind(Row.Names=rownames(impProt_D[[1]]),impProt_D[[1]])
impTotData = impProt_D[[1]]

for (i in 2:NumReps) { 
  cat( "Processing replicate ", i, "\n")
  impProt_D[[i]] = cbind(Row.Names=rownames(impProt_D[[i]]),impProt_D[[i]])
  rownames(impProt_D[[i]]) <- NULL
  impTotData= merge(impTotData,impProt_D[[i]],by="Row.Names",all=T)
  
}
rownames(impTotData) = impTotData[,1]
impTotData = impTotData[,2:(dim(impTotData)[2])]  
...
plot(PSMsM[[1]][,4],PSMsM[[1]][,6])

#Calculate percentage of missing values
#sum(is.na(TotData))/prod(dim(TotData))*100
sum(is.na(TotData))/prod(dim(TotData))*100
sum(is.na(impTotData))/prod(dim(impTotData))*100
...
write.csv(impTotData,file=paste("150mJ_Trizol_normData_imputed.csv"))
...
ChosenTable = impTotData # Compiled table of normalized data
