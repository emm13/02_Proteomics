...
plotDE <- function(results,x.norm,logfc.col,cpm.col= NULL,pval.col,fdr.col,pval=0.05,logfc=1,suf,out.dir=out.dir,cont.col.names){
  ...
  if(nrow(top) > 2){
    ...
    legend("bottomright",c("Adj.P.Value <= 0.05",paste("abs(logFC) >",logfc),paste("Adj.P.Value<=0.05 & abs(logFC) >",logfc),"Not significant"),fill=c("blue3","orange","green3","black"))
    ...
    text(10,4,"Up",col = "blue3")
    text(-10,4,"Down",col = "orange2")
    ...
    colnums = grep(cont.col.names,tolower(colnames(hmap.dat)))
