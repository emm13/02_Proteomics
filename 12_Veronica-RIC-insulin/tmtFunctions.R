...
runDE <- function(x.norm,des,contr.matrix,res.dir=out.dir,logfc=1,pval=0.05){
  ...
  for(c in 1:ncol(contr.matrix)){
    ...
    lim.all = rbind(lim.all,cbind(Contrast = cont.name,data.frame(topTable(efit,coef=c,n=Inf,p.value = 0.05))))
    ...
    # Plot DE genes
    #temp = plotDE(efit,x.norm,"logFC",NULL,"P.Value","adj.P.Val",pval=0.05,logfc=1,paste("",cont.name,sep="_"),res.dir, cont.col.names)
  }
  ...
  return(list(edgefit,efit,lim.all))
}
...
plotDE <- function(results,x.norm,logfc.col,cpm.col= NULL,pval.col,fdr.col,pval=0.05,logfc=1,suf,out.dir=out.dir,cont.col.names){
  ...
  p = grep(strsplit(suf,"\\_")[1],colnames(results))
  ...
  legend("bottomleft",c("Adj.P.Value <= 0.05",paste("abs(logFC) >",logfc),paste("Adj.P.Value<=0.05 & abs(logFC) >",logfc),"Not significant"),fill=c("blue3","orange","green3","black"))
  
  # Label a subset of points with the textxy function from the calibrate plot
  with(subset(top,abs(get(logfc.col))>=logfc), textxy(get(logfc.col), -log10(get(pval.col)), labs=gsub("_HUMAN","",rownames(top)), cex=.8,offset=0.6))
  
  # Add text for categorisation
  up.label = strsplit(suf,"vs")[[1]][1]
  text(-5,-2,paste("Up in",up.label,sep=" "),col = "blue3")
  text(5,-2,paste("Down in",up.label,sep=" "),col = "orange2")
  text((max(all[,logfc.col])-0)/2,max(-log10(all[,pval.col])),paste("Up in",up.label,sep=" "),col = "blue3")
  text((0+min(all[,logfc.col])/2),max(-log10(all[,pval.col])),paste("Down in",up.label,sep=" "),col = "orange2")
  ...
  if(length(k) > 2){
    ...
    hmap.dat.sub = hmap.dat.sub[order(hmap.dat.sub[,1],decreasing = T),]
    
    # Add a column colour option to distinguish the two comparison groups
    col.cols = c("green3","magenta3")[as.numeric(as.factor(sapply(strsplit(as.character(colnames(hmap.dat.sub)),"\\."),"[[",1)))] # Column side colours
    
    # Finally, draw the heatmap
    stats::heatmap(hmap.dat.sub,scale='row',Rowv=NA, Colv=NA,ColSideColors = col.cols, col=rev(morecols(50)),labRow=rownames(results)[k],labCol=rownames(x.norm$target),cexCol=1,keep.dendro=FALSE,main=paste("Top DE genes across",suf,sep=" "))
  }else{
